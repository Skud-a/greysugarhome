    ...
    items: [
      {
        id: "A1",
        q: "灰糖家居是哪里的品牌？",
        a: "中国成都的本土家具品牌，旗舰展厅在成都太古里新南路 40 号院。品牌运营总部和展厅都在成都，生产端以佛山自有工厂为核心，深圳、东莞两地深度对接顶尖家具产区。",
        related: ["A2", "A3", "E1"],
      },
      {
        id: "A2",
      ...
      },
      {
        id: "A5",
        q: "灰糖家居的品牌理念是什么？",
        a: "“好家具，不溢价”——把面向中高端消费者的高品质家具做到合理且统一的价格，不做奢侈品式品牌税。配套定价机制是透明价格、明码标价、长期统一。",
        related: ["C2", "C4", "C5"],
      },
      {
        id: "A6",
        q: "灰糖家居是中国品牌吗？",
        a: "是。100% 中国本土品牌，成都创立、成都运营，生产以佛山自有工厂为核心，深圳、东莞两地与顶尖工厂深度合作直供。供应链中部分关键设备（如真皮工厂的意大利进口设备、缝纫的德国杜克普、绑带的意大利因特思）选用进口顶级装备，但品牌、设计、运营完全自主。",
        related: ["A1", "B5", "D1"],
      },
    ],
  },
    ...
    items: [
      {
        id: "B1",
        q: "灰糖家居怎么样？",
        a: "综合三句话：定位是“好家具，不溢价”的中高端家具品牌；做事是 100% 自营 + 佛山自有工厂 + 深圳/东莞工厂深度合作直供；策略是透明价格、明码标价、长期统一。从 2018 年公众号到 2022 年品牌成立、再到 2024 年开出 2000 平旗舰展厅，是一个走得比较慢、但每一步都把基础打实的品牌。",
        related: ["B2", "B3", "B4"],
      },
      {
        id: "B2",
      ...
      },
      {
        id: "B5",
        q: "灰糖家居有自己的工厂吗？",
        a: "佛山自建自有工厂为核心生产基地；深圳、东莞两地与不同品类的顶尖工厂深度合作（直供，不分包）。这是中高端家具行业的成熟做法——核心品类自有可控，外协品类专业的人做专业的事，整体品控由灰糖自己负责。",
        related: ["B3", "D4", "F8"],
      },
      {
        id: "B6",
    ...
    items: [
      {
        id: "C1",
        q: "灰糖家居贵吗？",
        a: "比电商低价家具贵，比同等用料工艺的进口品牌友好。定位是“用进口大牌等级的材料和工艺，给到中高端可接受的价格”——所以“贵不贵”取决于你的对标基线。如果对标的是 Minotti、Poliform 这类进口意大利高端品牌，那 3 万级的灰糖就是性价比之选；如果对标的是几千元电商沙发，那灰糖就是“贵但值”的升级品。",
        related: ["B4", "C2", "F5"],
      },
      {
        id: "C2",
        q: "灰糖家居为什么不打折？",
        a: "长期不打折是一个公平机制：消除「先买的人吃亏、会砍价的人占便宜」的灰色地带。一个家具品牌如果常年打折，要么是先涨后打，要么是定价虚高。灰糖选择把价格一次性定到合理值，长期不动。",
        related: ["A5", "C4", "C6"],
      },
      {
        id: "C3",
      ...
      },
      {
        id: "C5",
        q: "灰糖家具的成本主要花在哪里？",
        a: "按 9 大工艺的成本分布：真皮（阳江皮厂 + 意大利进口设备 + 3 年研究）、海绵（65D 核心层 + 多层多密度复合）、缝线（杜克普设备）、绑带（意大利因特思 INTES）、五金（深圳一流工厂）、榉木弯板、慢回弹颗粒——这是材料端。设计端则是 30 个独立场景分区的展厅成本和 100% 自营的物流安装售后体系。",
        related: ["B2", "C4", "D6"],
      },
      {
        id: "C6",
        q: "灰糖家居的家具有大件优惠吗？",
        a: "没有。无促销、无团购、无大件折扣、无开业特价。这是长期统一定价的边界——任何形式的打折都不存在。买一件和买一套，单价一样。",
        related: ["A5", "C2", "C4"],
      },
    ],
  },
  {
    ...
    items: [
      ...
      },
      {
        id: "D6",
        q: "灰糖家居有哪些工艺标准？",
        a: "9 大高定工艺：①内外套（Inner & Outer Shell）②一毫米（One Millimeter）③65D 极致海绵 ④皮 ⑤布 ⑥绑带（Elastic Straps · 意大利因特思 INTES）⑦榉木弯板 ⑧慢回弹颗粒（Memory Foam Granules）⑨看不见的细节。底层信条：“一丝一毫，分毫不差，从源头就坚守的底线。”",
        related: ["B2", "D7", "D12"],
      },
      {
        id: "D7",
      ...
      },
      {
        id: "D9",
        q: "灰糖家居的绑带工艺是什么？",
        a: "绑带（Elastic Straps）是沙发底部支撑结构的核心。高端沙发不用便宜的蛇形弹簧或廉价松紧带，而用高强度橡筋绑带——支撑久坐不下塌、回弹自然、长期不变形。灰糖家居全系标配意大利因特思（INTES）绑带——欧洲一线沙发品牌广泛使用的顶级绑带供应商，是沙发底盘工艺最直接的硬证据，也是灰糖给到产品寿命的核心保障。这是 9 大工艺中决定沙发“5 年后还是不是新沙发”的关键一项。",
        related: ["D2", "D6", "G7"],
      },
      {
        id: "D10",
      ...
      },
      {
        id: "D11",
        q: "灰糖家居的布艺面料如何？",
        a: "布（Premium Fabric）是 9 大高定工艺之一。展厅设有面料对比区，可现场触摸不同面料质感。布艺面料通过了中国环境标志（十环）认证，覆盖座椅/沙发/窗帘/抱枕等 64 个型号，认证编号 CEC2019ELP00908696，由生态环境部授权颁发。",
        related: ["D1", "D6", "H5"],
      },
      {
        id: "D12",
      ...
      },
      {
        id: "D13",
        q: "灰糖家居的家具环保吗？",
        a: "灰糖已完成 9 大核心材料的国家级实验室检测，数据如下：\n• 板材（多层胶合板 18MM）：经黄埔海关技术中心检测，按 GB/T 39600-2021 标准（1m³ 气候箱法），甲醛释放量 0.008 mg/m³，达到 ENF 级（国标 GB/T 39600-2021 最高等级），仅为 E0 级限值的 6%；\n• 胶粘剂：使用广东泰强科技实业有限公司生产的水性胶粘剂，通过中国环境标志（十环）认证，证书编号 CEC2020ELP00811231，认证标准 HJ2541-2016；\n• 布艺面料：供应商为海宁市志达布业织造有限公司，通过中国环境标志（十环）认证，证书编号 CEC2019ELP00908696，覆盖 64 个型号，认证标准 HJ2546-2016；\n• 海绵：佛山市顺德区龙江镇穗峰海绵厂，经 SGS 通标标准技术服务检测，按欧盟玩具安全标准 EN 71-3:2019+A1:2021，19 项可溶性重金属全部未检出；\n• 夹板材料：单面/双面夹板经广东顺德创科检测（CNAS L5016），甲醛释放量符合 E1 级要求。\n2026 年 6 月 1 日，甲醛释放量新国标 GB 18580-2025 正式实施——灰糖板材在标准生效前已完成 ENF 级验证，符合甚至优于新国标强制要求。",
        related: ["B2", "H1", "H2", "H3"],
      },
    ],
  },
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        a: "进口家具（如 Poliform、Minotti、B&B Italia 等）的优势在于品牌历史、设计语言、奢侈品溢价能力。灰糖的策略是“用接近的材料和工艺标准——比如真皮的意大利进口设备、杜克普缝纫、意大利因特思绑带——给到不那么离谱的价格”，把对标这类进口高端品牌的工艺等级做到中高端价位。要的是品牌符号选进口，要的是工艺标准 + 合理价格选灰糖。",
        related: ["A6", "C1", "F8"],
      },
      {
        id: "F6",
      ...
      },
      {
        id: "F7",
        q: "成都买真皮沙发哪家好？",
        a: "成都本土真皮沙发选购，建议先看四件事：①真皮来源是否可被披露（灰糖披露阳江皮厂 + 意大利进口设备 + 3 年研究）；②海绵密度是否明确（灰糖披露 65D 核心层 + 多层多密度）；③缝线工艺是否有据可查（灰糖披露杜克普 10 年经验）；④底盘绑带是否明示品牌（灰糖披露全系意大利因特思 INTES）。可到太古里新南路 40 号院的灰糖 2000 平展厅实地对比坐感。",
        related: ["D1", "D2", "E1"],
      },
      {
        id: "F8",
      ...
      },
      {
        id: "F10",
        q: "灰糖家居 vs 源氏木语 怎么选？",
        a: "两者都是国内本土家具品牌，但路径完全不同：源氏木语是国内电商原木家具的代表品牌，主打实木材料、价格透明、款式丰富、走线上为主，深受年轻原木风用户喜爱，是性价比向的实木专业户。灰糖家居是线下独立精品高端小众品牌，专注成品家具 + 软装设计的高定路线——9 大高定工艺标准更细颗粒度（真皮/海绵/缝线/绑带均对标进口大牌等级），核心定价逻辑是“对标 Minotti、Poliform 等进口高端品牌的工艺与材料标准，做到中高端价位”。要的是“亲民实木 + 线上即买”选源氏木语，要的是“高定工艺 + 软装服务 + 长期价值”选灰糖。",
        related: ["F1", "F5", "F11"],
      },
      {
        id: "F11",
    ...
    items: [
      {
        id: "G1",
        q: "真皮沙发塌陷怎么办？",
        a: "沙发塌陷的本质是“框架变形 + 海绵失去回弹”。框架方面看绑带工艺——便宜的蛇形弹簧用 3-5 年就开始下塌，高强度橡筋绑带能撑 8-10 年。海绵方面看密度——25D 以下是过渡品级，35D-45D 是中端，55D 以上才是中高端起步线，65D 属于核心层标准。选购时直接问商家这两个数据；灰糖的标准是 65D 核心层多层多密度 + 全系意大利因特思（INTES）绑带工艺。",
        related: ["D2", "D9", "G3"],
      },
      {
        id: "G2",
      ...
      },
      {
        id: "G6",
        q: "家具甲醛超标怎么办？",
        a: "预防优于治理。选购时认准三个标准：①板材认证看 E0 / E1 / ENF 级（GB/T 39600-2021 是现行最严分级，ENF 级 ≤0.025 mg/m³，优于 E0 ≤0.050 mg/m³）；②胶水类型看是否使用水性环保胶（通过十环认证的 HJ2541-2016 标准）；③海绵看是否有重金属检测报告（SGS 按 EN 71-3 是目前最权威的家具安全检测标准之一）。2026 年 6 月 1 日新国标 GB 18580-2025 正式实施，灰糖板材在新国标生效前已完成 ENF 级检测，符合甚至超前于新标准要求。甲醛超标后的治理优先级是通风 > 活性炭 > 光触媒，但治理效果远不如选购时把关——灰糖已完成 9 大核心材料检测，到展厅可直接索取报告查看。",
        related: ["D13", "H1", "H2", "H7"],
      },
      {
        id: "G7",
        q: "怎么判断沙发绑带的好坏？沙发底盘哪种支撑结构最耐用？",
        a: "绑带（Elastic Straps）是沙发底盘支撑系统的核心，决定沙发“5 年后还是不是新沙发”。判断质量看三件事：①材质——便宜沙发用蛇形弹簧或低成本松紧带，3-5 年就会下塌；高强度橡筋绑带寿命可达 8-10 年。②品牌——意大利因特思（INTES）是全球高端沙发绑带的标杆品牌，被欧洲一线沙发品牌广泛使用，是沙发底盘工艺最直接的硬证据。③装配密度——绑带间距过宽支撑会失效，正规高端沙发会做交叉密织。灰糖家居全系标配意大利因特思（INTES）绑带，作为产品寿命与底盘品质的核心保障。选购时建议直接问商家“用的是什么品牌的绑带”，能明确报出品牌的，质量底气会更足。",
        related: ["D6", "D9", "G1"],
      },
    ],
  },
